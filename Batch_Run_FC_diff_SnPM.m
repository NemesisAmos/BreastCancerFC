            ...
            for tempRange=1:length(Distance_Range)
                ...
                for temp_freq=1:length(all_freq)
                    ...
                    for tempcluster=1:length(large_cluster)
                        ...
                        FCDataPath=strcat(LoadRoot,'\Results\Freq',all_freq{temp_freq},'_FC_FunImgARWSD',globalind,'CFB_',Distance_Range{tempRange},'\', zscore_fix{tempfix},thres_type{temp_thres},'_',zscore_fix{tempfix},'FC_Seed_',tempclustername,'\');
                        %construct data
                        ...
                        output_Dir=strcat(LoadRoot,'statistical_res\',globalsig,'\',zscore_fix{tempfix},thres_type{temp_thres},'_',zscore_fix{tempfix},'FC_Seed_',tempclustername,group_type{temp_group});
                        ...
                        for temp_subject=1:length(NC_subjects)
                            matlabbatch{1}.spm.tools.snpm.des.TwoSampT.scans1{temp_subject}=strcat(FCDataPath,'Group1\',zscore_fix{tempfix},'FC_Seed_',tempclustername,'_',NC_subjects{temp_subject},'.nii,1');
                        end
                        
                        if strcmp(group_type{temp_group},'_FCRI_low')
                            BrestCancer_subjects=subjlist(find(group==2 & FCRI<13 & medicated~=1));
                        elseif strcmp(group_type{temp_group},'_FCRI_high')
                            BrestCancer_subjects=subjlist(find(group==2 & FCRI>=13 & medicated~=1));
                        else
                            BrestCancer_subjects=subjlist(find(group==2 & medicated~=1));
                        end
                        ...
                        for temp_subject=1:length(BrestCancer_subjects)
                            matlabbatch{1}.spm.tools.snpm.des.TwoSampT.scans2{temp_subject}=strcat(FCDataPath,'Group2\',zscore_fix{tempfix},'FC_Seed_',tempclustername,'_',BrestCancer_subjects{temp_subject},'.nii,1');
                        end
                        
                        if strcmp(group_type{temp_group},'_FCRI_low')
                            matlabbatch{1}.spm.tools.snpm.des.TwoSampT.cov.c = age(find(FCRI==-1 | FCRI<13 & medicated~=1));
                        elseif strcmp(group_type{temp_group},'_FCRI_high')
                            matlabbatch{1}.spm.tools.snpm.des.TwoSampT.cov.c = age(find(FCRI==-1 | FCRI>=13 & medicated~=1));
                        else
                            matlabbatch{1}.spm.tools.snpm.des.TwoSampT.cov.c = age(find(medicated~=1));
                        end
                        
                        matlabbatch{1}.spm.tools.snpm.des.TwoSampT.cov.cname = 'Age';
                        ...
                        matlabbatch{3}.spm.tools.snpm.inference.WriteFiltImg.name = 'FC_signficant_positive_group_diff_SnPM';
                        ...
                        matlabbatch{4}.spm.tools.snpm.inference.WriteFiltImg.name = 'FC_signficant_negative_group_diff_SnPM';
