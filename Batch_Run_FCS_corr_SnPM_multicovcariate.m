...
excelroot='H:\breast_cancer\breast_cancer_subj_info_new.xlsx';
...
edu=behavioraldata(:,3);
FCRI=behavioraldata(:,4);
FCRtotal=behavioraldata(:,5);
        ...
        for temp_thres=1:length(thres_type)
            for tempfix=1:length(zscore_fix)
                for tempRange=1:length(Distance_Range)
                    ...
                    matlabbatch{1}.spm.tools.snpm.des.Corr1S.P=cell(length(subjlist),1);
                    for temp_subject=1:length(subjlist)
                        matlabbatch{1}.spm.tools.snpm.des.Corr1S.P{temp_subject}=strcat(FCSDataPath,zscore_fix{tempfix},thres_type{temp_thres},Distance_Range{tempRange},'_',subjlist{temp_subject},'.nii,1');
                    end    
                    ...
                    if strcmp(group_type{temp_group},'_FCRI_low')
                        matlabbatch{1}.spm.tools.snpm.des.Corr1S.cov(1).c = age(find(FCRI<13));
                        matlabbatch{1}.spm.tools.snpm.des.Corr1S.cov(2).c = edu(find(FCRI<13));
                    ...
                    elseif strcmp(group_type{temp_group},'_FCRI_high')
                        ...
                        matlabbatch{1}.spm.tools.snpm.des.Corr1S.cov(1).c = age;
                        matlabbatch{1}.spm.tools.snpm.des.Corr1S.cov(2).c = edu;
                        matlabbatch{1}.spm.tools.snpm.des.Corr1S.CovInt = temp_covint;
                        matlabbatch{1}.spm.tools.snpm.des.Corr1S.xblock = length(FCRI);
                    end
                    matlabbatch{1}.spm.tools.snpm.des.Corr1S.cov(1).cname = 'Age';    
                    matlabbatch{1}.spm.tools.snpm.des.Corr1S.cov(2).cname = 'Education'; 
